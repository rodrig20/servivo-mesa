...
class WebdriverAutoUpdate:
    ...
    def download_latest_version(self):
        ...
        latest_driver_zip = wget.download(download_url, out=self.driver_directory, bar=None)
        ...
        shutil.rmtree(latest_driver_zip[:-4])
    ...
    def get_local_driver_version(self):
        ...
        try:
            ...
            if os.path.isfile(chromedriver_executable):
                self.set_execution_bit_on_chromedriver(chromedriver_executable)
                self._unquarantine_chromedriver(chromedriver_executable)
                cmd_run = subprocess.run(args=[chromedriver_executable, "--version"], capture_output=True, text=True)
    ...
    def _unquarantine_chromedriver(self, chromedriver_executable: str):
        ...
        if self.is_unix():
            if self.is_mac():
                ...
                subprocess.run(args=["xattr", "-d", "com.apple.quarantine", chromedriver_executable], capture_output=True)
            else:
